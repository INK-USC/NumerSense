    ...
    for input in inputs:
        ...
        for token_id, token in enumerate(tokens):
            if args.model_type == "roberta":
                current_masked_indices.append(token[1:] in num_list)
            else:
                current_masked_indices.append(token in num_list)
        # if sum(current_masked_indices) == 0:
        # print(tokens)
        # print(current_masked_indices)
        # assert sum(current_masked_indices) >= 1 # at least one number here
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs, labels = mask_num_tokens(batch, tokenizer, args) if args.mlm else (batch, batch)
